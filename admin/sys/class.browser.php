<?php
   ...
   *
   * 2009-12-20 (by Luis Leiva):
   *  + Changed some const strings to match the old BrowserDetect JS script
...
class Browser {
    ...
    const BROWSER_OPERA_MINI = 'Opera Mini';                  // http://www.opera.com/mini/
		...
		const BROWSER_IE = 'IExplorer';                           // http://www.microsoft.com/ie/
		const BROWSER_POCKET_IE = 'Pocket IExplorer';             // http://en.wikipedia.org/wiki/Internet_Explorer_Mobile
    ...
    protected function setRobot($value=true) {
      $this->_is_robot = $value;
		...
		/**
		 * Determine if the browser is Flock or not
		 * @return boolean True if the browser is Flock otherwise false
		 */
		protected function checkBrowserFlock() {
			$retval = false;
			if( preg_match('/Flock/i',$this->_agent) ) {
			  $aresult = explode('/',stristr($this->_agent,'Flock'));
				$aversion = explode(' ',$aresult[1]);
				$this->setVersion($aversion[0]);
				$this->setBrowser(self::BROWSER_FLOCK);
				$retval = true;
			}
			return $retval;
		}
