...
package com.speedzinemedia.smt.mouse {
	...
	import com.speedzinemedia.smt.Asset;
	import com.speedzinemedia.smt.Maths;
    ...
    public class MouseView extends MouseController
    {   
		...
		// mask for non-single-point layers
		private var $maskLayers:Array = [
            Layers.id.REGISTRATION,
            Layers.id.STOP,
            Layers.id.CLUSTER
        ];
        ...
        {
            // mask layer will be composited with mouse coordinates
            layer.graphics.beginFill(0xFFFFFF);
            layer.graphics.drawRect(0,0, $screen.currWindow.width,$screen.currWindow.height);
        ...
        {   
            $maskLayers.forEach(function(elm:*, idx:int, arr:Array):void {
                drawMask($canvas[elm]);
            });
        ...
        {   
			...
			if (super.realTime && _leader) {
				ExternalInterface.call("window.smtAuxFn.doScroll", 
                    {   xpos:   q.x,
                        ypos:   q.y,
		...
		{ 
			...
			var basePath:String = ExternalInterface.call("window.smtAuxFn.getBase");
        ...
        {
            // always draw mask
            DrawUtils.drawCircle($canvas[Layers.id.MASK], p, $heatMapSize, Layers.getColor(Layers.id.MASK), 1);
            ...
            rp.name = "#" + super.count +"; time: "+Maths.roundTo(super.count/stage.frameRate, 2);
            addTooltipListeners(rp);
            // draw square
		...
		private function drawHesitation(p:Point, size:int):void
        {
            ...
            h.name = "stop: "+duration+" seconds @ (" + Math.round(p.x) + ", " + Math.round(p.y) + ")";
		...
		private function drawCentroid(p:Point):void 
        {
            ...
            c.graphics.lineStyle(10, cColor, .5);
            // add a circle to distinguish better from clicks
            DrawUtils.drawCircle(c, p, SIZE * .9, cColor, 0.7);
            //DrawUtils.drawStar(c, p, 18);
		...
		private function addTooltipListeners(s:Sprite, button:Boolean = false):void
        {
            s.buttonMode = button;
