...
var smtAuxFn = {
  ...
  overrideTrackingOptions: function(smtOptionsObj, customOptionsObj) 
  {
    for (var prop in smtOptionsObj) {
      if (customOptionsObj.hasOwnProperty(prop) && customOptionsObj[prop]) {
   ...
   * @return void
   * @param {string} type   message type: "log" (default), "info", or "error"
   * @param {mixed}  msg    message text (anything to trace)
   */
   ...
   * @return {object} document dimmensions - object with 2 properties: width {integer}, and height {integer}  
   */
  ...
  getPageSize: function()
  {
    ...
    // however, some browsers in some situations should return more space available
  ...
  browserDetect: {
    /**
     * Launches the browser detect script.
     * @return void
     */
  	init: function() 
    {
  	...
  	/**
     * Performs an identity search on provided data object.
     * @return {string}       browser identity     
     * @param {object} data   dataBrowser identity
     */
  	searchString: function(data) 
    {
  	...
  	/**
     * Performs a version search on provided data string.
    ...
    [
  		...
  		{ string: navigator.userAgent, subString: "Chrome",   identity: "ChromeOS"    } 
  	]
  },
  /** 
  ...
  array: {
     ...
     */
    indexOf: function(array, search, from) 
    {
      for (var i = (from || 0), total = array.length; i < total; ++i) {
